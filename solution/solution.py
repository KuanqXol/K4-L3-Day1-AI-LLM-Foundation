...
def _client_config_for_model(model: str) -> tuple[str | None, str | None]:
    return (
        os.getenv("GEMINI_API_KEY") or os.getenv("OPENAI_API_KEY"),
        os.getenv("GEMINI_BASE_URL") or os.getenv("OPENAI_BASE_URL"),
    )


def _completion_options_for_model(model: str) -> dict:
...
def call_openai(
    ...
    api_key, base_url = _client_config_for_model(model)
    client = OpenAI(
        api_key=api_key,
        base_url=base_url,
        timeout=20.0,
    ...
    response = client.chat.completions.create(
        ...
        **_completion_options_for_model(model),
    )
...
def streaming_chatbot() -> None:
    ...
    from openai import OpenAI
    api_key, base_url = _client_config_for_model(OPENAI_MODEL)
    client = OpenAI(
        api_key=api_key,
        base_url=base_url,
        timeout=20.0,
